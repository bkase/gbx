use crate::bus::{Bus, BusScalar, IoRegs};
use crate::exec::Exec;
use crate::ppu_stub::CYCLES_PER_FRAME;
use crate::{mmu, Core, Model, Scalar};
...
type BackendCore = Core<BackendExec, BackendBus>;
type ExecU8 = <BackendExec as Exec>::U8;
type ExecU16 = <BackendExec as Exec>::U16;

fn exec_from_u8(value: u8) -> ExecU8 {
    <BackendExec as Exec>::from_u8(value)
}

fn exec_from_u16(value: u16) -> ExecU16 {
    <BackendExec as Exec>::from_u16(value)
}

fn exec_to_u8(value: ExecU8) -> u8 {
    <BackendExec as Exec>::to_u8(value)
}

fn exec_to_u16(value: ExecU16) -> u16 {
    <BackendExec as Exec>::to_u16(value)
}

fn core_with_rom(rom: Arc<[u8]>) -> BackendCore {
    Core::new(backend_new_bus(Arc::clone(&rom)), backend_core_config(), Model::Dmg)
}

/// Builds a backend core with the provided program bytes at reset address 0x0100.
fn core_with_program(bytes: &[u8]) -> BackendCore {
    let mut rom = vec![0u8; 0x8000];
    let start = 0x0100;
    rom[start..start + bytes.len()].copy_from_slice(bytes);
    core_with_rom(Arc::from(rom.into_boxed_slice()))
}
...
fn take_serial(core: &mut BackendCore) -> String {
    backend_take_serial(core)
}

fn bus_mut(core: &mut BackendCore) -> &mut BusScalar {
    backend_bus_mut(core)
}

fn bus(core: &BackendCore) -> &BusScalar {
    backend_bus(core)
}

fn io_mut(core: &mut BackendCore) -> &mut IoRegs {
    &mut bus_mut(core).io
}

fn io(core: &BackendCore) -> &IoRegs {
    &bus(core).io
}

fn vram_mut(core: &mut BackendCore) -> &mut [u8; 0x2000] {
    &mut bus_mut(core).vram
}

fn vram(core: &BackendCore) -> &[u8; 0x2000] {
    &bus(core).vram
}
...
fn cpu_add8_flags() {
    ...
    for (a, b, expected, z, n, h, c) in cases {
        ...
        core.cpu.a = exec_from_u8(a);
        core.cpu.b = exec_from_u8(b);
        ...
        assert_eq!(exec_to_u8(core.cpu.a), expected, "ADD result mismatch");
...
fn cpu_adc8_flags() {
    ...
    for (a, imm, carry_in, expected, z, n, h, c) in cases {
        ...
        core.step_cycles(16);
        assert_eq!(exec_to_u8(core.cpu.a), expected, "ADC result mismatch");
...
fn cpu_sbc8_flags() {
    ...
    for (a, imm, carry_in, expected, z, n, h, c) in cases {
        ...
        core.step_cycles(16);
        assert_eq!(exec_to_u8(core.cpu.a), expected, "SBC result mismatch");
...
fn cpu_immediate_logic_flags() {
    ...
    core.cpu.a = exec_from_u8(0x33);
    core.cpu.f.from_byte(0xFF);
    core.step_cycles(16);
    assert_eq!(exec_to_u8(core.cpu.a), 0x03);
    assert!(core.cpu.f.h());
    assert!(!core.cpu.f.n());
    assert!(!core.cpu.f.c());
    ...
    core.cpu.a = exec_from_u8(0x30);
    core.cpu.f.from_byte(0);
    core.step_cycles(16);
    assert_eq!(exec_to_u8(core.cpu.a), 0x3F);
    assert!(!core.cpu.f.h());
    assert!(!core.cpu.f.n());
    assert!(!core.cpu.f.c());
    ...
    core.cpu.a = exec_from_u8(0xFF);
    core.cpu.f.from_byte(0xFF);
    core.step_cycles(16);
    assert_eq!(exec_to_u8(core.cpu.a), 0x00);
    assert!(core.cpu.f.z());
    assert!(!core.cpu.f.h());
    assert!(!core.cpu.f.n());
    assert!(!core.cpu.f.c());
...
fn cpu_cp_preserves_a_and_flags_match_sub() {
    ...
    core_sub.cpu.a = exec_from_u8(0x20);
    ...
    core_cp.cpu.a = exec_from_u8(0x20);
    core_cp.cpu.f.from_byte(0);
    core_cp.step_cycles(16);

    assert_eq!(exec_to_u8(core_cp.cpu.a), 0x20, "CP must not mutate A");
    assert_eq!(
...
fn inc_dec_hl_preserves_carry() {
    ...
    let addr = exec_from_u16(0xC000);
    let value = core.bus.read8(addr);
    assert_eq!(exec_to_u8(value), 0x0F);
    assert!(
        core.cpu.f.c(),
...
fn daa_adjusts_after_add_and_sub() {
    ...
    let mut core = core_with_program(&[0x27, 0x76]);
    core.cpu.a = exec_from_u8(0x15);
    core.cpu.f.from_byte(0);
    core.cpu.f.set_n(true);
    core.cpu.f.set_c(true);
    core.cpu.f.set_h(true);
    core.step_cycles(8);
    assert_eq!(exec_to_u8(core.cpu.a), 0xAF);
    assert!(core.cpu.f.c());
    assert!(!core.cpu.f.h());
    assert!(core.cpu.f.n());
}
...
fn serial_transfer_accumulates_output() {
    ...
    let data_addr = exec_from_u16(0xFF01);
    let ctrl_addr = exec_from_u16(0xFF02);

    core.bus.write8(data_addr, exec_from_u8(b'O'));
    core.bus.write8(ctrl_addr, exec_from_u8(0x81));
    core.step_cycles(8 * 512);
    core.bus.write8(data_addr, exec_from_u8(b'K'));
    core.bus.write8(ctrl_addr, exec_from_u8(0x81));
    core.step_cycles(8 * 512);
    ...
    assert_eq!(take_serial(&mut core), expected);
}
...
fn rotate_a_variants() {
    ...
    core.cpu.a = exec_from_u8(0x80);
    core.cpu.f.from_byte(0);
    core.cpu.f.set_c(true);
    core.step_cycles(8);
    assert_eq!(exec_to_u8(core.cpu.a), 0x01);
    assert!(core.cpu.f.c());

    // RRCA
    let mut core = core_with_program(&[0x0F, 0x76]);
    core.cpu.a = exec_from_u8(0x01);
    core.cpu.f.from_byte(0xFF);
    core.step_cycles(8);
    assert_eq!(exec_to_u8(core.cpu.a), 0x80);
    assert!(core.cpu.f.c());
    assert!(!core.cpu.f.z());
...
fn add_hl_rr_flag_behavior() {
    ...
    core.cpu.set_hl(exec_from_u16(0x0FFF));
    core.cpu.set_bc(exec_from_u16(0x0001));
    core.cpu.f.from_byte(0);
    core.cpu.f.set_z(true);

    core.step_cycles(12);
    assert_eq!(exec_to_u16(core.cpu.hl()), 0x1000);
    assert!(core.cpu.f.h());
    assert!(!core.cpu.f.c());
    assert!(!core.cpu.f.n());
...
fn add_sp_e8_flag_behavior() {
    ...
    core.cpu.sp = exec_from_u16(0xFFF8);
    core.cpu.f.from_byte(0xFF);
    core.step_cycles(20);
    assert_eq!(exec_to_u16(core.cpu.sp), 0x0000);
    assert!(!core.cpu.f.z());
    assert!(!core.cpu.f.n());
    assert!(core.cpu.f.h());
    assert!(core.cpu.f.c());
...
fn ld_hl_sp_plus_e8_sets_flags() {
    ...
    core.cpu.f.from_byte(0);
    core.step_cycles(16);
    assert_eq!(exec_to_u16(core.cpu.hl()), 0xFFF0);
    assert!(!core.cpu.f.z());
    assert!(!core.cpu.f.n());
    assert!(core.cpu.f.h());
    assert!(core.cpu.f.c());
...
fn push_pop_af_masks_low_bits() {
    ...
    core.cpu.a = exec_from_u8(0xAB);
    core.cpu.f.from_byte(0xF0);
    core.step_cycles(64);
    assert_eq!(exec_to_u8(core.cpu.a), 0xAB);
...
fn jr_condition_cycles() {
    ...
    assert_eq!(exec_to_u8(core.cpu.a), 0xAA);
    ...
    assert_eq!(exec_to_u8(core.cpu.a), 0xBB);
...
fn timers_wrap() {
    ...
    let mut core = core_with_rom(Arc::from(rom.into_boxed_slice()));
    io_mut(&mut core).set_tac(0b101); // enable timer, 16-cycle frequency
    io_mut(&mut core).set_tma(0xAC);
    io_mut(&mut core).set_tima(0xFF);
    let initial_if = io(&core).if_reg();

    core.timers.step(16, &mut core.bus);

    assert_eq!(io(&core).tima(), 0xAC, "TIMA should reload from TMA");
    assert_eq!(
        io(&core).if_reg(),
        initial_if | 0x04,
...
fn ppu_line_and_vblank_progression() {
    ...
    assert_eq!(
        io(&core).read(IoRegs::LY),
        144,
        "LY should be 144 at VBlank start"
    );
    assert_eq!(
        io(&core).read(IoRegs::STAT) & 0x03,
        0x01,
...
fn ppu_stat_mode_interrupt_edges() {
    ...
    io_mut(&mut core).write(IoRegs::STAT, 0x20);
    io_mut(&mut core).set_if(0);
    core.ppu.reset();
    ...
    core = core_with_rom(Arc::from(vec![0x00u8; 0x8000].into_boxed_slice()));
    io_mut(&mut core).write(IoRegs::LCDC, 0x80);
    io_mut(&mut core).write(IoRegs::STAT, 0x08);
    io_mut(&mut core).set_if(0);
    ...
    core = core_with_rom(Arc::from(vec![0x00u8; 0x8000].into_boxed_slice()));
    io_mut(&mut core).write(IoRegs::LCDC, 0x80);
    io_mut(&mut core).write(IoRegs::STAT, 0x10);
    io_mut(&mut core).set_if(0);
    for _ in 0..144 {
        core.ppu.step(LINE_CYCLES, &mut core.bus);
    }
...
fn ppu_lyc_coincidence_interrupt() {
    ...
    let mut core = core_with_rom(Arc::from(rom.into_boxed_slice()));
    io_mut(&mut core).write(IoRegs::LCDC, 0x80);
    io_mut(&mut core).write(IoRegs::LYC, 10);
    io_mut(&mut core).write(IoRegs::STAT, 0x40);
    io_mut(&mut core).set_if(0);
    ...
    assert_eq!(io(&core).read(IoRegs::LY), 10);
    assert_eq!(
        io(&core).read(IoRegs::STAT) & 0x04,
        0x04,
    ...
    io_mut(&mut core).write(IoRegs::LYC, 12);
    core.ppu.step(LINE_CYCLES, &mut core.bus);
    assert_eq!(io(&core).read(IoRegs::LY), 12);
    assert_eq!(io(&core).read(IoRegs::STAT) & 0x04, 0x04);
    assert_eq!(
        io(&core).if_reg() & 0x02,
        0x02,
...
fn lcd_off_freezes_state() {
    ...
    assert!(io(&core).read(IoRegs::LY) > 0);
    ...
    core.ppu.step(LINE_CYCLES, &mut core.bus);
    assert_eq!(io(&core).read(IoRegs::LY), 0, "LY should stay frozen");
...
fn bg_render_unsigned_tile_data() {
    let mut core = core_with_rom(Arc::from(vec![0x00u8; 0x8000].into_boxed_slice()));
    io_mut(&mut core).write(IoRegs::LCDC, 0x91);
    io_mut(&mut core).write(IoRegs::BGP, 0xE4);

    let tile_offset = 16usize;
    vram_mut(&mut core)[tile_offset] = 0xFF;
    vram_mut(&mut core)[tile_offset + 1] = 0x00;
...
fn bg_render_signed_tile_data() {
    let mut core = core_with_rom(Arc::from(vec![0x00u8; 0x8000].into_boxed_slice()));
    io_mut(&mut core).write(IoRegs::LCDC, 0x81);
    io_mut(&mut core).write(IoRegs::BGP, 0xE4);

    let tile_offset = 0x0FF0usize; // 0x8FF0 - 0x8000
    vram_mut(&mut core)[tile_offset] = 0x00;
    vram_mut(&mut core)[tile_offset + 1] = 0xFF;
...
fn bg_render_scroll_wraps() {
    ...
    io_mut(&mut core).write(IoRegs::LCDC, 0x91);
    io_mut(&mut core).write(IoRegs::BGP, 0xE4);
    io_mut(&mut core).write(IoRegs::SCX, 252);
    io_mut(&mut core).write(IoRegs::SCY, 248);

    let tile1 = 16usize;
    vram_mut(&mut core)[tile1] = 0xFF;
    vram_mut(&mut core)[tile1 + 1] = 0x00;

    let tile2 = 32usize;
    vram_mut(&mut core)[tile2] = 0x00;
    vram_mut(&mut core)[tile2 + 1] = 0xFF;

    let tile3 = 48usize;
    vram_mut(&mut core)[tile3] = 0xFF;
    vram_mut(&mut core)[tile3 + 1] = 0xFF;

    let map_base = 0x1800usize;
    vram_mut(&mut core)[map_base + 31 * 32 + 31] = 0x01;
    vram_mut(&mut core)[map_base + 31 * 32] = 0x02;
...
proptest! {
    ...
    fn prop_adc_matches_reference(a in any::<u8>(), b in any::<u8>(), carry_in in any::<bool>()) {
        ...
        assert_eq!(exec_to_u8(core.cpu.a), expected);
    ...
    fn prop_cp_matches_sub_flags(a in any::<u8>(), b in any::<u8>()) {
        ...
        core_sub.cpu.a = exec_from_u8(a);
        core_sub.cpu.f.from_byte(0);
        core_sub.step_cycles(16);
        let flags_sub = core_sub.cpu.f.to_byte();

        let mut core_cp = core_with_program(&[0xFE, b, 0x76]);
        core_cp.cpu.a = exec_from_u8(a);
        core_cp.cpu.f.from_byte(0);
        core_cp.step_cycles(16);

        assert_eq!(exec_to_u8(core_cp.cpu.a), a);
